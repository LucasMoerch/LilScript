...
let string_of_stmt = function
  | Keybinds kbs ->
      "KEYS:\n" ^ String.concat "\n"
        (List.map (fun { action; key } ->
          "  " ^ string_of_key_name action ^ ": " ^ key) kbs)

(* token to string for --tokens mode *)
let string_of_token = function
  | CONSTANTS -> "CONSTANTS" | COLON -> "COLON"   | COMMA -> "COMMA"
  | LBRACKET  -> "LBRACKET"  | RBRACKET -> "RBRACKET"
  | NEWLINE   -> "NEWLINE"   | INDENT -> "INDENT"  | DEDENT -> "DEDENT"
  | EOF       -> "EOF"       | PLUS -> "PLUS"      | MINUS -> "MINUS"
  | MULTIPLY  -> "MULTIPLY"  | DIVIDE -> "DIVIDE"  | LPAREN -> "LPAREN"
  | RPAREN    -> "RPAREN"    | ARENA -> "ARENA"    | SPAWN -> "SPAWN"
  | PLAYERS   -> "PLAYERS"   | KEYS -> "KEYS"      | COLOR -> "COLOR"
