...
let save_arena editor =
  ...
  Array.iter (fun row ->
    Array.iter (fun tile ->
      output_char oc (char_of_tile tile)
    ) row;
    output_char oc '\n'
  ) editor.arena.grid;
  close_out oc
...
let handle_input editor = function
  | `Arrow `Up ->
      { editor with cursor_y = max 0 (editor.cursor_y - 1) }

  | `Arrow `Down ->
      { editor with cursor_y = min (editor.arena.height - 1) (editor.cursor_y + 1) }

  | `Arrow `Left ->
      { editor with cursor_x = max 0 (editor.cursor_x - 1) }

  | `Arrow `Right ->
      { editor with cursor_x = min (editor.arena.width - 1) (editor.cursor_x + 1) }
