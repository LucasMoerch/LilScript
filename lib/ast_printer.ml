...
let rec print_expr level expr =
  ...
  | Econst sc -> (
      match sc with
      | SCint i ->
          Printf.printf "%sSCint %d\n" (indent level) i
      | SCfloat f ->
          Printf.printf "%sSCfloat %f\n" (indent level) f
      | SCbool b ->
          Printf.printf "%sSCbool %b\n" (indent level) b
      | SCstring s ->
          Printf.printf "%sSCstring \"%s\"\n" (indent level) s
    )

  | Evar id ->
      Printf.printf "%sEvar %s\n" (indent level) id.id
...
let print_const level c =
  ...
  match c.value with
  | Cint i ->
      Printf.printf "%sCint %d\n" (indent (level + 1)) i
  | Cfloat f ->
      Printf.printf "%sCfloat %f\n" (indent (level + 1)) f
  | Cstring s ->
      Printf.printf "%sCstring \"%s\"\n" (indent (level + 1)) s
  | Cbool b ->
      Printf.printf "%sCbool %b\n" (indent (level + 1)) b
  | Cexpr e ->
